...
bool DocExtractor::LoadWordStemmer(void)
{
   const wchar_t* keyName = L"SYSTEM\\CurrentControlSet\\Control\\ContentIndex\\Language\\English_US";
   HKEY hKey = 0;
   if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyName, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
   {
      wchar_t wordStemmerClass[MAX_PATH];
      DWORD size = sizeof(wordStemmerClass);
      DWORD type = 0;
      LSTATUS status = RegQueryValueEx(hKey, L"StemmerClass", 0, &type, (BYTE *) wordStemmerClass, &size);
      RegCloseKey(hKey);

      if (status == ERROR_SUCCESS || type == REG_SZ)
      {
         // locate word stemmer
         CLSID clsidWordStemmer;
         if (FAILED(CLSIDFromString(wordStemmerClass, &clsidWordStemmer)))
            return false;

         // create word stemmer
         if (FAILED(CoCreateInstance(clsidWordStemmer, 0, CLSCTX_ALL, IID_IStemmer, (void **) &pWordStemmer_)))
            return false;

         // initialize word stemmer
         BOOL bLic = 0;
         if (FAILED(pWordStemmer_->Init(100, &bLic)))
         {
            pWordStemmer_->Release();
            return false;
         }
...
void DocExtractor::_Word(const wstring& word)
{
   MyWordStemmerSink ss(*this);
   pWordStemmer_->GenerateWordForms(word.c_str(), word.length(), &ss);
}

void DocExtractor::_AltWord(const wstring& word)
{
   MyWordStemmerSink ss(*this);
   pWordStemmer_->GenerateWordForms(word.c_str(), word.length(), &ss);
}
