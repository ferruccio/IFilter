...
public :
   ...
   bool LoadWordStemmer(void);
   void Extract(const TCHAR* filename);

   virtual void Word(const wstring& word) {}
   virtual void AltWord(const wstring& word) {}
   virtual void AltWordForm(const wstring& word) {}

   void _Word(const wstring& word);
   void _AltWord(const wstring& word);
